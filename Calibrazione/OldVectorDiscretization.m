function [SVector_Euler,SVector_Milstein] = OldVectorDiscretization(k,theta,epsilon,mu,rho,v,r,S,N)
  ...
  for j = 1:N 
    ...
    vE = max((vE + k*((theta -((rho*epsilon*(mu-r))/k)) - vE)*delta + epsilon*sqrt(vE*delta)*Z_v(j)),0);
  end
  for j = 1:N 
    ...
    vM = max((vM + k*((theta -((rho*epsilon*(mu-r))/k)) - vM)*delta + epsilon*sqrt(vM*delta)*Z_v(j) + (0.25*(epsilon^2)*delta)*((Z_v(j)^2)-1)),0);
  end
end